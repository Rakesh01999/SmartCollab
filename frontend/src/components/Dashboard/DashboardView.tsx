    ...
    <div className="flex-1 p-6 space-y-8 max-w-7xl mx-auto w-full animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl">
        <div>
          ...
          <p className="text-xs md:text-sm lg:text-base text-slate-400 mt-1">
        ...
        {kpis.map((kpi, idx) => (
          ...
          >
            <div className="space-y-1">
              <span className="text-xs md:text-sm lg:text-base font-semibold text-slate-400 tracking-wider block">{kpi.title}</span>
              <span className="text-xl md:text-2xl lg:text-3xl font-bold text-white block">{kpi.value}</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 glass-panel border border-slate-800/80 p-6 rounded-2xl flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-sm md:text-base lg:text-lg text-slate-200">Tasks by Priority</h3>
            <p className="text-xs md:text-sm text-slate-500 mt-0.5">Distribution of tasks sorted by urgency.</p>
          </div>
          ...
          <div className="text-xs md:text-sm text-slate-500 border-t border-slate-900 pt-3 flex justify-between">
        ...
        <div className="lg:col-span-4 glass-panel border border-slate-800/80 p-6 rounded-2xl flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-sm md:text-base lg:text-lg text-slate-200">Task Status Distribution</h3>
            <p className="text-xs md:text-sm text-slate-500 mt-0.5">Task lifecycle breakdown.</p>
          </div>
          ...
          <div className="relative flex items-center justify-center py-6">
            ...
            <div className="absolute text-center">
              <span className="text-xl md:text-2xl lg:text-3xl font-bold text-white block">
                {totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0}%
              </span>
              <span className="text-[10px] md:text-xs font-bold text-emerald-400 tracking-wide uppercase">Completed</span>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-2 text-center text-xs md:text-sm border-t border-slate-900 pt-4">
            <div>
              <span className="block text-slate-400 font-semibold">Todo</span>
              <span className="block text-sm md:text-base font-bold text-slate-300 mt-0.5">{todoCount}</span>
            </div>
            <div>
              <span className="block text-blue-400 font-semibold">In Progress</span>
              <span className="block text-sm md:text-base font-bold text-blue-300 mt-0.5">{inProgressCount}</span>
            </div>
            <div>
              <span className="block text-emerald-400 font-semibold">Done</span>
              <span className="block text-sm md:text-base font-bold text-emerald-300 mt-0.5">{completedTasks}</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 glass-panel border border-slate-800/80 p-6 rounded-2xl flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-sm md:text-base lg:text-lg text-slate-200">Project Progress Overview</h3>
            <p className="text-xs md:text-sm text-slate-500 mt-0.5">Summary of major team deliverables.</p>
          </div>
            ...
            {projectSummaries.map((proj, idx) => (
              <div key={proj._id || idx} className="space-y-1">
                <div className="flex justify-between items-center text-xs md:text-sm">
                  <span className="font-semibold text-slate-300 truncate max-w-[150px] md:max-w-[200px]">{proj.name}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] md:text-xs font-semibold text-slate-500 w-24 md:w-28 text-right truncate">
                    {proj.deadlineText}
                  </span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 glass-panel border border-slate-800/80 p-6 rounded-2xl">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="font-bold text-sm md:text-base lg:text-lg text-slate-200">Member Workload Summary</h3>
              <p className="text-xs md:text-sm text-slate-500 mt-0.5">Assigned tasks and completed work ratio per member.</p>
            </div>
            <button
              onClick={() => onViewChange('team')}
              className="text-xs md:text-sm text-indigo-400 hover:text-indigo-300 font-bold cursor-pointer"
            >
              View Team
            </button>
          </div>
                ...
                <div key={item.member._id || idx} className="flex items-center gap-4 p-2 bg-slate-900/30 rounded-xl border border-slate-900">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <span className="text-[10px] md:text-xs text-slate-500 w-24 md:w-28 text-right truncate">
        ...
        <div className="lg:col-span-6 glass-panel border border-slate-800/80 p-6 rounded-2xl">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="font-bold text-sm md:text-base lg:text-lg text-slate-200">Recent Activities</h3>
              <p className="text-xs md:text-sm text-slate-500 mt-0.5">Logs of recent updates on tasks and projects.</p>
            </div>
            <button
